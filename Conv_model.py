...
class convnet(nn.Module):
    ...
    def __init__(self, input_size, v1_size, v1_orientation_number, v4_size, v4_stride, v4_orientation_number, phis, 
        ...
        self.v4_weight = nn.Conv3d(self.v1_orientation_number, self.v4_orientation_number, (self.v1_orientation_number, self.v4_size, self.v4_size), stride = self.v4_stride, bias = False)
        self.v4_weight.weight.data = self.init_gaussian_weights()
        
        # Initialising decision layer
        self.decision = nn.Linear(self.v4_orientation_number * self.v4_dimensions * self.v4_dimensions, 2, bias = False)
        self.decision.weight = torch.nn.Parameter(torch.zeros((2, self.v4_orientation_number * self.v4_dimensions * self.v4_dimensions)))
        ...
        self.v4_weight_before = nn.Conv3d(self.v1_orientation_number, self.v4_orientation_number, (self.v1_orientation_number, self.v4_size, self.v4_size), stride = self.v4_stride, bias = False)
    ...
    def plot_angle_performance(self, angle_number, color):
        ...
        for i in tqdm(range(angle_number)):
            ...
            for j in range(self.training_size):
                ...
                kernel = self.generate_location_gabor(angle, phases[j//10], sfs[j//10], int((self.v1_dimensions - 1)/2), int((self.v1_dimensions - 1)/2))
...
# Helper functions
    ...
    def generate_gabor(self, size, theta, phi, lamda, gamma = 0.5):
        ...
        self.lamda = lamda # Wavelength of sinusoidal component so determines sf
