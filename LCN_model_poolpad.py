...
class LCN(nn.Module):
    ...
    def __init__(self, input_size, v1_size, v1_orientation_number, v4_size, v4_stride, v4_orientation_number, phis_sfs, 
        ...
        self.decision.weight = torch.nn.Parameter(
            torch.zeros((2, self.v4_orientation_number))) 
            ...
            for i in range(self.v1_orientation_number):
                for j in range(self.phis_sfs):
                    for k in range(self.v1_dimensions ** 2):
                        ...
                        kernel = self.generate_gabor(self.v1_size, theta, 0, lamda, gamma = self.v1_gamma) 
                        kernel = kernel * self.v1_rescale
...
#                         kernel = kernel + noise
    ...
    def forward(self, x):
        ...
        x = F.pad(x, (self.v1_padding, self.v1_padding, self.v1_padding, self.v1_padding))
        ...
        # Locally connected multiplication, then summing over space to space to create activation maps with dimensions 1 x v1_orientation_number * phis_sfs x v1_dimensions x v1_dimensions
        ...
        out = F.pad(pools, (self.v4_padding, self.v4_padding, self.v4_padding, self.v4_padding))
        out = out.unfold(1, self.v4_size, self.v4_stride).unfold(2, self.v4_size, self.v4_stride) 
        ...
        # Spatial mean pool
        v4_spatial_pool = v4_pool.sum([1, 2]) / (self.v4_dimensions * self.v4_dimensions)
        ...
        out = v4_spatial_pool.view(1, self.v4_orientation_number) 
...
#                 break
            ...
            if i % 500 == 0 or i == iterations - 1:
                ...
                self.otc_curve(v1_position_1 = int((self.v1_dimensions - 1) / 2), v1_position_2 = int((self.v1_dimensions - 1) / 2), v4_position_1 = int((self.v4_dimensions - 1) / 2), v4_position_2 = int((self.v4_dimensions - 1) / 2))
                ...
                if i == iterations - 1:
                    for a in range(self.v1_dimensions):
                        for b in range(self.v1_dimensions):
                            self.otc_curve(v1_position_1 = a, v1_position_2 = b, v4_position_1 = int((self.v4_dimensions - 1) / 2), v4_position_2 = int((self.v4_dimensions - 1) / 2))
                    ...
                    for c in range(self.v4_dimensions):
                        for d in range(self.v4_dimensions):
                            self.otc_curve(v1_position_1 = int((self.v1_dimensions - 1) / 2), v1_position_2 = int((self.v1_dimensions - 1) / 2), v4_position_1 = c, v4_position_2 = d)
...
#         self.v4_tuning_curve() 
    ...
    def v4_tuning_curve(self):
        ...
        with torch.no_grad():
            for i in range(len(x)):
                ...
                test = F.pad(test, (self.v1_padding, self.v1_padding, self.v1_padding, self.v1_padding))
                ...
                pools_after = F.pad(pools_after, (self.v4_padding, self.v4_padding, self.v4_padding, self.v4_padding))
                pools_before = F.pad(pools_before, (self.v4_padding, self.v4_padding, self.v4_padding, self.v4_padding))
                ...
                for k in range(self.v4_orientation_number):
                    for a in range(self.v4_dimensions):
                        for b in range(self.v4_dimensions):
                            out_after = (out_a[:, int((self.v4_dimensions - 1) / 2), int((self.v4_dimensions - 1) / 2), :] * self.v4_weight[k, :, a, b, :]).sum([-1, 0])
                            v4_pool_after[k][a][b] = out_after

                            out_before = (out_b[:, int((self.v4_dimensions - 1) / 2), int((self.v4_dimensions - 1) / 2), :] * self.before_v4weight[k, :, a, b, :]).sum([-1, 0])
    ...
    def otc_curve(self, v1_position_1, v1_position_2, v4_position_1, v4_position_2):
        ...
        # Ensure v1_position is between 0 and v1_dimensions
        ...
        for i in range(self.v1_orientation_number):
            ...
            for j in range(self.phis_sfs):
                ...
                curve = self.results[i, j, v1_position_1, v1_position_2, :]
                initial = self.initial_tuning_curves[i, j, v1_position_1, v1_position_2, :]
